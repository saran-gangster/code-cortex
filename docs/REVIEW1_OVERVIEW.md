...
- **Development result:** matched ImageNet E2 reached AP50 0.0692 and AP50:95 0.0234, compared with E1 at 0.0120 and 0.0024. E2 is the leading candidate on the single development recording only.
...
Validated data → recording-root split → matched E1/E2 training → development evaluation → versioned evidence/API → review console. VisDrone is a planned external RGB-only evaluation.
...
**Limits:** the development comparison supports E2, but final-test metrics are unavailable. One development recording is weak evidence for generalization. The missing-state fallback, release threshold, empty-scene behavior and autonomous-flight safety remain unproven.
